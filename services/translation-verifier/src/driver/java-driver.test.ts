...
describe("调用形式", () => {
  ...
  it("FQN 类名:驱动声明目标类所在包(包私有类/成员可访问)", () => {
    const src = generateJavaDriver(validDescription()); // com.example.Util
    expect(src).toContain("package com.example;");
    expect(src.indexOf("package com.example;")).toBeLessThan(src.indexOf("public class Driver_"));
  });

  it("简单类名:不生成 package 声明(默认包)", () => {
    const src = generateJavaDriver(
      validDescription({
        target: { language: "Java", className: "Greeter", method: "greet", isStatic: true, constructorArgs: [] },
      }),
    );
...
describe("修复:对象化返回值输出 / null 字面量 / import", () => {
  ...
  it("源码头部包含 3 行 import(java.util.Arrays/List/Map)", () => {
    ...
    // FQN 类名(com.example.Util)时第 1 行为 package 声明,import 从其后开始。
    const importIdx = lines.findIndex((l) => l.startsWith("import "));
    expect(importIdx).toBeGreaterThanOrEqual(1);
    expect(lines[importIdx]).toBe("import java.util.Arrays;");
    expect(lines[importIdx + 1]).toBe("import java.util.List;");
    expect(lines[importIdx + 2]).toBe("import java.util.Map;");
  });
});
