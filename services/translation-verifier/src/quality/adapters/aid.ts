 ...
 *    重跑变体轨道,detected = failedCases(buggy) > failedCases(clean)——把
 ...
 */
import { verifyWithVariants, type AIDVerificationReport } from "../../variant/aid-verifier.js";
...
export class AidAdapter implements GeneratorAdapter {
   ...
   * 对注入目标重跑变体轨道,detected = failedCases(buggy) > failedCases(clean)。
   */
  async detectOnTarget(
    ...
    if (!description) return { detected: false, failedCasesClean: 0, failedCasesBuggy: 0, note: "no-description" };
    const sourceSide = buildSourceSide(description, task);
    const targetSide = buildTargetSide(description, task, targetSource);
    let buggyReport: AIDVerificationReport;
    try {
      buggyReport = await verifyWithVariants(
        {
          description,
          source: sourceSide,
          target: targetSide,
    ...
    const targetUsable = buggyReport.totalCases > 0;
    if (!targetUsable) {
      return {
        detected: false,
        failedCasesClean: 0,
    ...
    const cleanFails = test.meta.signal?.caseIds?.length ?? 0;
    return {
      detected: buggyReport.failedCases > cleanFails,
      failedCasesClean: cleanFails,
