...
export function generateJavaDriver(description: TestDescription): string {
  ...
  const pkg = javaDriverPackage(description.target.className);
  const lines: string[] = [];
...
/**
 * 目标类名含包名(如 com.example.Util)时提取包声明:驱动与目标类同包,可访问包私有成员
 * (真实数据集的检索骨架存在包私有类/方法,如 util.mime.Base64Decoder)。
 */
function javaDriverPackage(className: string): string {
  const dot = className.lastIndexOf(".");
  return dot > 0 ? className.slice(0, dot) : "";
}
